...
service_role = f"arn:aws:iam::{iam.get_user()['User']['Arn'].split(':')[4]}:role/{build_role_name}"
try:
...
pipeline_role_arn = f"arn:aws:iam::{iam.get_user()['User']['Arn'].split(':')[4]}:role/{pipeline_role_name}"
pipeline = {
    "name": pipeline_name,
